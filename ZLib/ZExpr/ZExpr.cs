	...
	public class ZExpr
	{
		...
		public ZExpr(string expression, ZTask task)
		{
			...
			if ( !expression.StartsWith("'") && !expression.StartsWith("="))
				this.expression = "=" + expression;
			string [] tags = null;
			if (task != null && task.parentStep != null && task.parentStep.tags != null)
			{
		...
		static string tagBegPattern			= @"(///<\{(?<" + ExprTags.tagIdx0 + @">[\.A-Z0-9]*)(,(?<" + ExprTags.tagIdx1 + @">\d))?\})";
...
#region taggedPatterns
			...
			foreach (Match match in matches)
			{
				...
				if (match.Groups.Count > 0)
				{
					try
					{
						// we'll either have an index number, or a path
