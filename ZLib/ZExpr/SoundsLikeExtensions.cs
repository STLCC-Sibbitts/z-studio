...
namespace ZLib
{
    public static class SoundsLike
    {
        ...
        public static int LevenshteinDistance(this string s, string t)
        {
			...
			// if either of the strings is 0 length, return the length of the other
            if (n == 0 || m == 0)
                return Math.Max(m, n);
			// store an index into s in the first column
            for (int i = 0; i <= n; i++)
            {
                d[i, 0] = i;
            }
			// store an index into t in the first row
            for (int i = 0; i < m; i++)
            {
                d[0, i] = i;
            }
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < m; j++)
                {
					...
					// looking to see if the character found in s is anywhere in t
		...
		// TODO: look at working both ends against the middle and use the Leven algorithm to figure out
		// what to do.
		...
		public static decimal LevDistPct(this string s, string t)
		{
