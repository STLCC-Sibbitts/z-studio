...
namespace JWC
{
	...
	public class MruStripMenu
	{
		...
		protected string			settingKeyName;
		...
		/// <param labelName="recentFilesList">Current list of mru.</param>
		/// <param labelName="maxEntries">The maximum number of items on the MRU list.</param>
		public MruStripMenu(ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, string [] recentFiles, int maxEntries)
		{
			Init(recentFileMenuItem, clickedHandler, recentFiles, maxEntries);
		}

		protected void Init(ToolStripMenuItem recentFileMenuItem, ClickedHandler clickedHandler, 
				string [] recentFiles, int maxEntries)
		{
		...
		protected void OnClick(object sender, System.EventArgs e)
		{
			...
			string[] miFilename = menuItem.Filename.Split(' ');
			string filename = miFilename[miFilename.Length - 1];

			clickedHandler(MenuItems.IndexOf(menuItem) - StartIndex, filename);
		}
...
//					MenuItems[i].Text = FixupEntryname(startNumber, MenuItems[i].Text.Substring(startNumber == 10 ? 5 : 4));
		...
		{
			string pathname = Path.GetDirectoryName(filename) + "\\" + Path.GetFileName(filename);
